...
async def _create_rubric(
    rubric: CreateRubric, pg_session: AsyncSession, es_session: AsyncElasticsearch
) -> ShowRubric | None:
    async with pg_session.begin():
        rubric_dal = RubricDAL(pg_session, es_session)
        new_rubric = await rubric_dal.create_rubric(
            text=rubric.text, rubrics=rubric.rubrics, created_date=rubric.created_date
        )
    ...
    return ShowRubric(
        id=new_rubric.id,
        text=new_rubric.text,
        rubrics=new_rubric.rubrics,
        created_date=new_rubric.created_date,
    )


async def _get_20_rubrics_by_text(
    text: str, pg_session: AsyncSession, es_session: AsyncElasticsearch
) -> List[ShowRubric] | None:
    async with pg_session.begin():
        rubric_dal = RubricDAL(pg_session, es_session)
...
async def _delete_rubric_by_id(
    id: int, pg_session: AsyncSession, es_session: AsyncElasticsearch
) -> DeletedRubric | None:
    async with pg_session.begin():
        rubric_dal = RubricDAL(pg_session, es_session)
